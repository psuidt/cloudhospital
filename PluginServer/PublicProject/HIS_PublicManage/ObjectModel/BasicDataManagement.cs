...
namespace HIS_PublicManage.ObjectModel
{
    ...
    public class BasicDataManagement:AbstractObjectModel
    {
        ...
        /// 获取手术字典
        /// </summary>
        /// <returns>返回手术字典</returns>
        public DataTable GetOperation()
        {
            DataTable dt = NewDao<IPublicManageDao>().GetOperation();
            return dt;
        }
        /// <summary>
        /// 病人信息相关，带“全部”项的下拉选项
    ...
    {
        国籍,教育程度,民族,婚姻状况,职业,关系,性别,血型,入院情况,麻醉方式,地区编码,病人来源,预交金支付方式,挂号支付方式,护理级别, 饮食种类,手术级别, 切口愈合等级, 损伤中毒外部原因
    }
    /// <summary>
    /// 科室数据源类型
    /// </summary>
