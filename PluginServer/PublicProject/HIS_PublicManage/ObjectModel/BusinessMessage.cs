            ...
            switch (msgType)
            {
                case MessageType.门诊新处方:
                    ...
                    if (isCheck)
                    {
                        ...
                        mesage.MessageContent = string.Format("已收到{0}患者(科室：{1},{2}医生)的处方，请及时配药发药！",
                ...
                case MessageType.住院新医嘱:
                    ...
                    if (isCheck)
                    {
                        ...
                        mesage.MessageTitle = string.Format("【{0}有新医嘱" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】", patDt.Rows[0]["DeptName"]);
                ...
                case MessageType.护理级别:
                    ...
                    if (isCheck)
                    {
                        // 获取业务消息包含的科室以及病人信息
                        DataTable patDt = NewDao<IPublicManageDao>().GetPatientInfo(Tools.ToInt32(msgDic["PatListID"], 0));
                        mesage.MessageTitle = string.Format("【{0}病人护理级别变更" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】", patDt.Rows[0]["PatName"]);
                ...
                case MessageType.病人新入院:
                    ...
                    if (isCheck)
                    {
                        // 获取业务消息包含的科室以及病人信息
                        DataTable patDt = NewDao<IPublicManageDao>().GetPatientInfo(Tools.ToInt32(msgDic["PatListID"], 0));
                        mesage.MessageTitle = string.Format("【{0}有新病人" + DateTime.Now.ToString("yyyy-MM-dd HH:mm") + "】", patDt.Rows[0]["DeptName"]);
