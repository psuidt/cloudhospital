    ...
    public class SqlDrugStoreInfoDao: AbstractDao,IDrugStoreInfoDao
    {
        ...
        public DataTable GetIPDrugBillDetail(Dictionary<string, string> condition)
        {
            string strSql = @"SELECT  1 AS chk,
                                ...
                                p.NursingLever,
                                p.Age,
                                dbo.fnGetDeptName(p.CurrDeptID) deptName,
                                p.Times,
                                dbo.fnGetPatTypeName(p.PatTypeID) patTypeName,
                                dbo.fnGetEmpName(p.CurrNurseID) NurseName,
                                a.PatDeptID ,
                                a.PatDoctorID ,
                        ...
                        FROM    IP_DrugBillHead k LEFT JOIN   IP_DrugBillDetail a ON a.BillHeadID=k.BillHeadID
                                ...
                                LEFT JOIN (SELECT * FROM BaseDictContent WHERE ClassId=1017) bd   
                                ON p.EnterSituation=bd.Code 
            ...
            strSql = strSql + strWhere.ToString();
            strSql = strSql + " ORDER BY a.PatListID,a.ItemID,a.OrderGroupID,a.CostDate Desc";
            return oleDb.GetDataTable(strSql);
        }
